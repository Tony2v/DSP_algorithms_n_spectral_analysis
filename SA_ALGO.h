...
public:
	vector<af::array> output_SA; // выход —ј
...
const vector <af::array>&  SA::process(const af::array& in)
{
	...
	{
		double mu = SA_settings.Fband / SA_settings.Ti; // скорость модул€ции дл€ Ћ„ћ сигнала
		...
		for (int i = 0; i < dist_size; i++)
		{
			...
			double Fmin_high = (SA_settings.Ti - (latency[i][0] - SA_settings.Blank))*mu; // частота минимльной обрабатываемой дальности дл€ данного строба приема (выше чем частота макс дальности)
			double Fmax_low = (SA_settings.Ti - (latency[i][1] - SA_settings.Blank))*mu;  // частота макс обрабатываемой дальности дл€ данного строба приема (ниже чем частота мин дальности)
			
			double k = nfft_size[i] / N_strobe; // коэффициент, учитывающий во сколько раз дополненна€ нул€ми посл-ть отсчетов больше чем кол-во поступающих отчетов в стробе приема

			double Fstep = 1 / (t_input[i].second - t_input[i].first) / k; // шаг по сетке частот —ј

			F_min_max_step.push_back(std::make_tuple(Fmin_high, Fmax_low, Fstep));

			double min_R_sample = Fmin_high * (t_input[i].second - t_input[i].first) * k;  // номер отсчета части спектра — которого требуетс€ производить запись, двига€сь в сторону нул€
			double max_R_sample = Fmax_low * (t_input[i].second - t_input[i].first) * k;   // номер последнего отсчета части спектра ƒќ которого требуетс€ производить запись, двига€сь в сторону нул€
